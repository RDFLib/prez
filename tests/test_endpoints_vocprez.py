...
def test_vocab_listing(test_client: TestClient):
    response = test_client.get(
        f"/v/vocab?_mediatype=text/anot+turtle&_profile=vocpub:schemes-list"
    )
...
def test_concept_narrowers(
    ...
    response = test_client.get(
        f"/v/vocab/{concept_scheme_curie}/{concept_curie}/narrowers?_mediatype=text/anot+turtle&_profile=https://w3id.org/profile/vocpub"
    )
    response_graph = Graph(bind_namespaces="rdflib").parse(data=response.text)
    expected_graph = Graph().parse(
        Path(__file__).parent
