...
class TestRemoteSparqlTimeout:
    ...
    async def test_send_query_with_timeout_param(self, remote_repo, mock_async_client):
        ...
        with patch.object(
            settings, "sparql_timeout_param_name", "timeout"
    ...
    @pytest.mark.asyncio
    async def test_send_query_without_timeout_param(
    ...
    async def test_sparql_get_with_timeout_param(self, remote_repo, mock_async_client):
        ...
        with patch.object(
            settings, "sparql_timeout_param_name", "maxExecutionTime"
        ), patch.object(settings, "sparql_timeout", 60), patch.object(
            settings, "sparql_endpoint", "http://test-endpoint.com"
        ):
    ...
    async def test_sparql_post_with_timeout_param(self, remote_repo, mock_async_client):
        ...
        with patch.object(
            settings, "sparql_timeout_param_name", "timeout"
        ), patch.object(settings, "sparql_timeout", 45), patch.object(
            settings, "sparql_endpoint", "http://test-endpoint.com"
        ):
    ...
    async def test_different_timeout_param_names(self, remote_repo, mock_async_client):
        ...
        for param_name in timeout_params:
            with patch.object(
                settings, "sparql_timeout_param_name", param_name
    ...
    @pytest.mark.asyncio
    async def test_timeout_error_handling_with_nice_message(
        self, remote_repo, mock_async_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_timeout_error_without_param_name(
        self, remote_repo, mock_async_client
    ):
        ...
        with patch.object(settings, "sparql_timeout_param_name", None), patch.object(
            settings, "sparql_timeout", 45
        ):
