...
class CatPrezDataset(PrezModel):
    ...
    def _get_properties(self) -> List[Dict]:
        ...
        for uri, prop in props_dict.items():
            if uri in CatPrezDataset.hidden_props:
                continue
            elif uri in CatPrezDataset.main_props:
        ...
        properties.extend(
            sorted(
                main_props,
                key=lambda p: self._sort_within_list(p, CatPrezDataset.main_props),
            )
        )
