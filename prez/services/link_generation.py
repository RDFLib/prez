...
async def add_prez_links(graph: Graph, repo: Repo):
    ...
    for uri, klasses in uri_to_klasses.items():
        if klasses:  # need class to know which endpoints can deliver the class
            await _link_generation(uri, repo, klasses, graph)
    ...
    # many node shapes to one endpoint; multiple node shapes can point to the endpoint
        ...
        for result in results:
            ...
            for solution in result[1]:
                ...
                hl = endpoints_graph_cache.value(predicate=ONT.hierarchyLevel, subject=result[0])
                if not hl:
                    raise ValueError(
                        f"Endpoint {result[0]} has no hierarchy level")  # TODO validate endpoint nodes with SHACL
                components = list(settings.endpoint_structure[:int(hl)])
                ...
                members_link = None
                if len(components) < len(list(settings.endpoint_structure)):
                    members_link = object_link + "/" + settings.endpoint_structure[len(components)]
