...
from prez.services.query_generation.cql import get_wkt_from_coords, format_coordinates_as_wkt


def generate_bbox_filter(bbox: List[float], filter_crs: str) -> (GraphPatternNotTriples, List[TriplesSameSubjectPath]):
    ...
    tssp_list = []
    tssp_list.append(TriplesSameSubjectPath.from_spo(subject, IRI(value=GEO.hasGeometry), geom_bn_var))
    tssp_list.append(TriplesSameSubjectPath.from_spo(geom_bn_var, IRI(value=GEO.asWKT), geom_lit_var))
