...
async def listing_function(
    ...
    count_query = temp_listing_count(listing_item, endpoint_uri)
...
async def listing_function_new(
    ...
    runtime_values = {"limit": per_page, "offset": (page - 1) * per_page, "parent_1": parent_uri}
    ...
    query_str = shacl_parser.sparql

    # pull the subselect out of the query string
    subselect = find_instances(shacl_parser.main_where_ggps, SubSelect)[0]  # assume there's only one subselect
    subselect.solution_modifier = None  # remove the limit and offset from the subselect so that we can get a count
    count_query = temp_listing_count(subselect, target_class)
    ...
    item_graph, _ = await repo.send_queries(
        rdf_queries=[count_query, query_str],
        tabular_queries=[],
    )
    if "anot+" in prof_and_mt_info.mediatype:
        await _add_prez_links(item_graph, repo)
